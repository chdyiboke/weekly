...
this从中删除参数 `Type`。如果 `Type`没有显式声明的this参数，则结果为`Type`。否则，this将从`Type`中创建一个没有参数的新函数类型。泛型将被移除，只有最后的重载签名才会传到新的函数类型中。
...
在上面的例子中，参数 methods 在对象 makeObject 中具有上下文类型，其包括 ThisType<D & M> 和因此this在此方法 `methods` 的指向是 { x: number, y: number } 和 { moveBy(dx: number, dy: number): number }。请注意，methods 属性的类型如何在方法中成为 this 类型的推断目标和来源。
